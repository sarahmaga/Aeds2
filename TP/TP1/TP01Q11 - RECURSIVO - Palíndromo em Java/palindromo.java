...
class palindromo {
    ...
    public static boolean isPalindromo(String s, int i) {
        ...
        // Metodo recursivo
        if (i < s.length() / 2) { // condicao de parada

            if (s.charAt(i) != s.charAt(s.length() - (i + 1))) { // condicao eh falsa (compara o caracter da esquerda
                                                                 // com
                                                                 // o da direita sucessivamente)
                resp = false;
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numEntrada; i++) {
            ...
            if (isPalindromo(entrada[i], 0) == false) {
