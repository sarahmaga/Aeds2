...
class Filme {
     ...
     */
    public static void swap(int m, int l, Filme[] array) {
        Filme temp = array[m];
        array[m] = array[l];
        array[l] = temp;
    ...
    public static void quicksort(Filme[] filmes, int n) {
        quicksort(filmes, 0, n - 1);
    }
     ...
     * @param esq    comeco do array
     * @param dir    final do array
     */
    public static void quicksort(Filme filmes[], int esq, int dir) {
        int i = esq, j = dir;
        Filme pivo = filmes[(esq + dir) / 2];
        System.out.println(pivo.getNome());
        while (i <= j) {
            while (filmes[i].getNome().compareTo(pivo.getNome()) < 0) {
                numeroDeComparacoes += 1;
                if (filmes[i].getSituacao().compareTo(pivo.getSituacao()) < 0) {
                    i++;
                } else {
                    i++;
                }
            }
            while (filmes[j].getNome().compareTo(pivo.getNome()) > 0) {
                numeroDeComparacoes += 1;
                if (filmes[j].getSituacao().compareTo(pivo.getSituacao()) > 0) {
                    j--;
                } else {
                    j--;
                }
            ...
            if (i <= j) {
                ...
                i++;
                j--;
                numeroDeMovimentacoes += 3;
            }

        }
        if (esq < j) {
            quicksort(filmes, esq, j);
        }
        if (i < dir) {
            quicksort(filmes, i, dir);
        }
...
class Main {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Criacao do log
        File log = new File("705113_quicksort.txt");
        log.createNewFile();
        BufferedWriter writer = new BufferedWriter(new FileWriter(log));
        writer.append("705113\t");
        // Execucao do algoritmo de ordenacao
        inicio = Filme.now();
        Filme.quicksort(filmes, numEntrada);
        fim = Filme.now();
        for (int i = 0; i < numEntrada; i++) {
      //      filmes[i].imprimir_2();
        }
