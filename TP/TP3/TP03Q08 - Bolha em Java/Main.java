...
class Filme {
    ...
    public static Filme[] bolha(Filme filmes[], int n) {
        boolean houveTroca = true;
        for (int i = 0; i < (n - 1) && houveTroca; i++) {
            houveTroca = false;
            for (int j = 0; j < n - (i + 1); j++) {
                ...
                } else if (filmes[j].getDuracao() == filmes[j + 1].getDuracao()) {
                    if (filmes[j].getNome().compareTo(filmes[j + 1].getNome()) > 0) {
                        swap(j, j + 1, filmes);
                        houveTroca = true;
                        numeroDeMovimentacoes += 3;
                    }
...
class Main {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Criacao do log
        File log = new File("705113_bolha.txt");
        log.createNewFile();
