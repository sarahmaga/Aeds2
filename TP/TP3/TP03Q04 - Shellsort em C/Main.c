...
void shellsort2(Movie arr[], int num)
{
    int i, j, k;
    Movie *tmp = (Movie *)malloc(sizeof(Movie) * 200);
    for (i = num / 2; i > 0; i = i / 2)
    {
        for (j = i; j < num; j++)
        {
            for (k = j - i; k >= 0; k = k - i)
            {
                if (strcmp(arr[k + i].original_language, arr[k].original_language) >= 0)
                {
                    break;
                }
                else
...
int main()
{
    ...
    FILE *log;
    log = fopen("705113_shellsort.txt", "w");
