...
class Filme {
    ...
    public static Filme[] selecao(Filme filmes[], int n) {
        // Filme[] filmesOrdenado = new Filme [n];
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
            for (int j = i + 1; j < n; j++) {
                if (filmes[menor].getTituloOriginal().compareTo(filmes[j].getTituloOriginal()) > 0) {
                    menor = j;
...
class Main {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Criacao do log
        File log = new File("705113_selecao.txt");
        log.createNewFile();
        ...
        inicio = Filme.now();
        Filme.selecao(filmes, numEntrada);
        fim = Filme.now();
        for (int i = 0; i < numEntrada; i++) {
            filmes[i].imprimir_2();
        }
