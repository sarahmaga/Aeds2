...
// Movie getMaior(Movie *array, int n) {
//     Movie maior = array[0];

//     for (int i = 0; i < n; i++) {
//         if(maior.duration < array[i].duration){
//             maior.duration = array[i].duration;
//         }
//     }
//     return maior;
// }
...
void countingsort(Movie *array, int n) {
    ...
    for(int i = 0; i < n; array[i] = ordenado[i], i++, numComparacoes++);
}
// -------------------------------------------------------------------------------- //
int main()
{
    ...
    FILE *log;
    log = fopen("705113_coutingsort.txt", "w");
    fprintf(log, "%s", "705113   ");
    fprintf(log, "%d", numComparacoes);
    fprintf(log, "%s", "    ");
    fprintf(log, "%d", numMovimentacoes);
    fprintf(log, "%s", "    ");
    fprintf(log, "%lf", ((double)time) / ((CLOCKS_PER_SEC / 1000)));
    fclose(log);
    return EXIT_SUCCESS;
}