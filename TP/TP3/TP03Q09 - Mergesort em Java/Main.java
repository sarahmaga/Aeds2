...
class Filme {
            ...
            try (// Setando o leitor de arquivos
                ...
                while (!linha.contains("<section class=\"content_score\">")) {
                    ...
                    // Metodo para pegar as palavras chave
                        ...
                        // Declaraco de variaveis, aux sao Strings auxiliares
                        String aux = "";
                        // As palavras chave serao primeiramente armazenadas num filmesList
    ...
    /**
     * 
     * @param m      = maior elemento
     * @param l      = menor elemento
     * @param filmes = filmes de elementos
     */
    public static void swap(int m, int l, Filme[] filmes) {
        Filme temp = filmes[m];
        filmes[m] = filmes[l];
        filmes[l] = temp;
    ...
    public static void mergeSort(Filme[] filmes, int tam) {
        mergeSort(filmes, 0, tam - 1);
    }

    public static void mergeSort(Filme[] filmes, int esq, int dir) {
        if (esq < dir) {
            int meio = (esq + dir) / 2;
            mergeSort(filmes, esq, meio);
            mergeSort(filmes, meio + 1, dir);
            intercalar(filmes, esq, meio, dir);
        }

    }

    public static void intercalar(Filme[] filmes, int esq, int meio, int dir) {
        int i, j, k;
        int n1 = meio - esq + 1;
        int n2 = dir - meio;
        Filme[] L, R;
        L = new Filme[n1];
        R = new Filme[n2];
        // Copia os dados para a temp de filmes L[] e R[]
        for (i = 0; i < n1; i++)
            L[i] = filmes[esq + i].clonaFilme();
        for (j = 0; j < n2; j++)
            R[j] = filmes[meio + 1 + j].clonaFilme();
        // Faz o merge de temp de volta no array
        i = 0;
        j = 0;
        k = esq;
        while (i < n1 && j < n2) {
            if (L[i].orcamento < R[j].orcamento) {
                filmes[k] = L[i].clonaFilme();
                i++;
                numeroDeComparacoes++;
                numeroDeMovimentacoes++;
            } else if (L[i].orcamento == R[j].orcamento) {
                if (L[i].nome.compareTo(R[j].nome) < 0)
                    filmes[k] = L[i].clonaFilme();
                i++;
                numeroDeComparacoes++;
                numeroDeMovimentacoes++;
        ...
        // Copia o restante dos elemento de L[] se existerem
        while (i < n1) {
            filmes[k] = L[i].clonaFilme();
            i++;
            k++;
        ...
        // Copia o restante dos elemento de R[] se existerem
        while (j < n2) {
            filmes[k] = R[j].clonaFilme();
            j++;
            k++;
...
class Main {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Criacao do log
        File log = new File("705113_mergesort.txt");
        log.createNewFile();
        ...
        inicio = Filme.now();
        Filme.mergeSort(filmes, numEntrada);
        fim = Filme.now();
        for (int i = 0; i < numEntrada; i++) {
            filmes[i].imprimir_2();
        }
