...
int minIndex(Movie *lista, int i, int j)
{
    ...
    numComparacoes++;
    if (strcmp(lista[i].original_title, lista[k].original_title) < 0)
    {
        k = i;
    }
    return k;
...
void selecaoRec(Movie *lista, int n, int index)
{
    if (index == n)
        return;
...
int main()
{
    ...
    clock_t tempo = clock();
    selecaoRec(movies, count_movies, 0);
    tempo = clock() - tempo;
