...
class Filme {
     ...
     */
    public static boolean pesquisaBinaria(Filme[] filmes, String entrada) {
        boolean existe = false;
        int comparacoes = 0;
        int esq = 0, dir = filmes.length - 1, meio = 0;
        // Declaraco de variaveis

        // Comeco do metodo de pesquisa binaria
        while (esq <= dir) {
            ...
            if (filmes[meio].getNome().equals(entrada)) {
                existe = true;
                esq = filmes.length;
            } else if (filmes[meio].getNome().compareTo(entrada) < 0) {
                esq = meio + 1;
            } else {
                dir = meio - 1;
            }
...
class Main {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        // Criacao do log
        File log = new File("705113_binaria.txt");
        log.createNewFile();
        ...
        for (int i = 0; i < numPesquisa; i++) {
            ...
            Filme atual = filmes[i];

            // Execucao do algoritmo de ordenacao
            inicio = atual.now();

            existe = Filme.pesquisaBinaria(filmes, pesquisa[i]);
            if (existe == true) {
            ...
            fim = atual.now();
            writer.append((fim - inicio) / 1000.0 + " s.\t");
            writer.append(Filme.numeroDeComparacoes + "\t");
